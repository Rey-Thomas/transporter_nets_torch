...
class TransporterAgent:
    ...
    def train(self, dataset, writer=None):
        ...
        if 'no-depth-transporter' in self.name:
            loss0 = self.attention.train(img[:,:,0:3], p0, p0_theta)
        else:
            loss0 = self.attention.train(img, p0, p0_theta)

        if isinstance(self.transport, Attention):
            if 'no-depth-transporter' in self.name:
                loss1 = self.transport.train(img[:,:,0:3], p1, p1_theta)
            else:
                loss1 = self.transport.train(img, p1, p1_theta)
        else:
            if 'no-depth-transporter' in self.name:
                loss1 = self.transport.train(img[:,:,0:3], p0, p1, p1_theta)
            else:
    ...
    def validate(self, dataset, writer=None):  # pylint: disable=unused-argument
        ...
        for _ in range(n_iter):
            ...
            if 'no-depth-transporter' in self.name:
                loss0 += self.attention.test(img[:,:,0:3], p0, p0_theta)
            else:
                loss0 += self.attention.test(img, p0, p0_theta)
            if isinstance(self.transport, Attention):
                if 'no-depth-transporter' in self.name:
                    loss1 += self.transport.test(img[:,:,0:3], p1, p1_theta)
                else:
                    loss1 += self.transport.test(img, p1, p1_theta)
            else:
                if 'no-depth-transporter' in self.name:
                    loss1 += self.transport.test(img[:,:,0:3], p0, p1, p1_theta)
                else:
    ...
    def act(self, obs, info=None, goal=None):  # pylint: disable=unused-argument
        ...
        if 'no-depth-transporter' in self.name:
            pick_conf = self.attention.forward(img[:,:,0:3])
        else:
        ...
        if 'no-depth-transporter' in self.name:
            place_conf = self.transport.forward(img[:,:,0:3], p0_pix)
        else:
...
class NoDepthTransporterAgent(TransporterAgent):
