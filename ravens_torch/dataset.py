...
class Dataset:
    ...
    def __init__(self, path, depth_use='Usual'):
    ...
    def load(self, episode_id, images=True, cache=False):
        ...
        def Adabins(episode_id, fname):
            ...
            # pretrained_path = "A:/ARIA/MVA/Object recognition/Project/transporter_nets_torch/pretrained/AdaBins_nyu.pt"
            # model, _, _ = model_io.load_checkpoint(pretrained_path, model)
            from infer import InferenceHelper
            from PIL import Image

            model = InferenceHelper(dataset='nyu')
            ...
            print(type(color))
            print(color.size)
            bin_edges, predicted_depth = model(img)
        ...
        for fname in sorted(os.listdir(path)):
            if f'{episode_id:06d}' in fname:
                ...
                if self.depth_use == 'Usual':
                    depth = load_field(episode_id, 'depth', fname)
                if self.depth_use == 'Adabins':
                    depth = Adabins(episode_id, fname)
...
def load_data(FLAGS, depth_use, only_test=False):
    ...
    if only_test:
        return Dataset(test_path, depth_use)

    return Dataset(train_path,depth_use), Dataset(test_path, depth_use)
