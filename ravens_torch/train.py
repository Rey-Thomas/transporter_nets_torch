...
def main(unused_argv):
    ...
    train_dataset, test_dataset = load_data(FLAGS,depth_use='Usual')#, depth_use='Adabins')
    ...
    for train_run in range(FLAGS.n_runs):
        ...
        while agent.total_steps < FLAGS.n_steps:
            for _ in range(FLAGS.interval):
                ...
                # cmap, hmap = utils.get_fused_heightmap(obs, cameras.RealSenseD415.CONFIG, np.array([[0.25, 0.75], [-0.5, 0.5], [0, 0.28]]), 0.003125)
                # print(hmap)
                # print(np.shape(hmap))
